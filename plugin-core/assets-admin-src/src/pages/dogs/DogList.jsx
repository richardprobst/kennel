...
function DogList() {
	...
	const fetchDogs = async () => {
		...
		try {
			...
			setError(err.message || __('Erro ao carregar cães.', 'canil-core'));
		} finally {
			setLoading(false);
		}
	};
	...
	const handleDelete = async (id) => {
		if (!window.confirm(__('Tem certeza que deseja excluir este cão?', 'canil-core'))) {
			return;
		}
		...
		try {
			...
			setError(err.message || __('Erro ao excluir cão.', 'canil-core'));
		}
	};
	...
	return (
		<div className="canil-dog-list">
			<div className="canil-page-header">
				...
				<Button variant="primary" onClick={() => navigate('/dogs/new')}>
				...
				<CardBody>
					...
					{loading ? (
						...
						<div className="canil-empty-state">
							<p>{__('Nenhum cão encontrado.', 'canil-core')}</p>
							<Button variant="primary" onClick={() => navigate('/dogs/new')}>
							...
							<table className="wp-list-table widefat fixed striped">
								<thead>
									<tr>
										...
										<th>{__('Nascimento', 'canil-core')}</th>
