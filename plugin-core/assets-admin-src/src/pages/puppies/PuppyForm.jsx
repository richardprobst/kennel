...
function PuppyForm() {
	...
	const fetchLitters = async () => {
		try {
			const response = await apiFetch({ path: '/canil/v1/litters/dropdown' });
	...
	const fetchPuppy = async () => {
		...
		try {
			...
			setError(err.message || __('Erro ao carregar filhote.', 'canil-core'));
		} finally {
			setLoading(false);
		}
	};
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (data.litter_id) data.litter_id = parseInt(data.litter_id, 10);
			...
			if (isEditing) {
				...
				setSuccess(__('Filhote atualizado com sucesso!', 'canil-core'));
			} else {
				await apiFetch({
			...
			setError(err.message || __('Erro ao salvar filhote.', 'canil-core'));
		} finally {
			setSaving(false);
		}
	};
	...
	return (
		<div className="canil-puppy-form">
			<div className="canil-page-header">
				...
				<Button variant="secondary" onClick={() => navigate('/puppies')}>
			...
			<form onSubmit={handleSubmit}>
					...
					<CardBody>
						<div className="canil-form-row">
							...
							<TextControl
								...
								required
								help={__('Ex: Macho 1, Fêmea 2', 'canil-core')}
							/>
						</div>
						<div className="canil-form-row">
							<TextControl
						...
						<div className="canil-form-row">
							<TextControl
								label={__('Número de Registro', 'canil-core')}
				...
				<Card>
					...
					<CardBody>
						<div className="canil-form-row">
							<TextControl
								label={__('Peso ao Nascer (gramas)', 'canil-core')}
								type="number"
								min="0"
								step="0.1"
							...
							/>
							<TextControl
								label={__('Ordem de Nascimento', 'canil-core')}
								type="number"
								min="1"
						...
						<TextareaControl
							label={__('Observações do Nascimento', 'canil-core')}
