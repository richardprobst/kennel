...
function LitterList() {
	...
	const fetchLitters = async () => {
		...
		try {
			...
			setError(err.message || __('Erro ao carregar ninhadas.', 'canil-core'));
		} finally {
			setLoading(false);
		}
	};
	...
	const handleDelete = async (id) => {
		if (!window.confirm(__('Tem certeza que deseja excluir esta ninhada?', 'canil-core'))) {
			return;
		}
		...
		try {
			...
			setError(err.message || __('Erro ao excluir ninhada.', 'canil-core'));
		}
	};
	...
	return (
		<div className="canil-litter-list">
			<div className="canil-page-header">
				...
				<Button variant="primary" onClick={() => navigate('/litters/new')}>
				...
				<CardBody>
					...
					{loading ? (
						...
						<div className="canil-empty-state">
							<p>{__('Nenhuma ninhada encontrada.', 'canil-core')}</p>
							<Button variant="primary" onClick={() => navigate('/litters/new')}>
								{__('Adicionar Primeira Ninhada', 'canil-core')}
							</Button>
						</div>
					) : (
						<>
							<table className="wp-list-table widefat fixed striped">
								<thead>
									<tr>
										<th>{__('Nome/Letra', 'canil-core')}</th>
										<th>{__('Status', 'canil-core')}</th>
										<th>{__('Data Cobertura', 'canil-core')}</th>
										<th>{__('Data Prevista', 'canil-core')}</th>
										<th>{__('Nascidos', 'canil-core')}</th>
								...
								<tbody>
									{litters.map((litter) => (
										<tr key={litter.id}>
											...
											<td>{getStatusLabel(litter.status)}</td>
											<td>{litter.mating_date || '-'}</td>
											<td>{litter.expected_birth_date || '-'}</td>
											<td>
