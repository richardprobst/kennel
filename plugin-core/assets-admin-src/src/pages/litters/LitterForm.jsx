...
function LitterForm() {
	...
	const matingTypeOptions = [
		...
		{
			label: __( 'Inseminação (fresco)', 'canil-core' ),
			value: 'artificial_fresh',
		},
		{
			label: __( 'Inseminação (congelado)', 'canil-core' ),
	...
	}, [ isEditing, fetchDogs, fetchLitter ] );
	...
	const handleSubmit = async ( e ) => {
		...
		try {
			...
			if ( data.dam_id ) {
				data.dam_id = parseInt( data.dam_id, 10 );
			}
			if ( data.sire_id ) {
				data.sire_id = parseInt( data.sire_id, 10 );
			}
			...
			if ( isEditing ) {
				...
				setSuccess(
					__( 'Ninhada atualizada com sucesso!', 'canil-core' )
				);
			} else {
				await apiFetch( {
			...
			setError(
				err.message || __( 'Erro ao salvar ninhada.', 'canil-core' )
			);
		} finally {
			setSaving( false );
		}
					...
					<CardBody>
						...
						<div className="canil-form-row">
							...
							<TextControl
								...
								help={ __(
									'Calculado automaticamente: cobertura + 63 dias',
									'canil-core'
								) }
							/>
						</div>
